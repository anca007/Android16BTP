...
import com.example.eni_shop.databinding.TemplateLineArticleBinding

class ArticleAdapter(val articleList: List<Article>, val listener: (article: Article) -> Unit) :
    Adapter<ArticleAdapter.ArticleVH>() {

    class ArticleVH(val binding: TemplateLineArticleBinding) : ViewHolder(binding.root)

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ArticleVH {
        val binding = TemplateLineArticleBinding.inflate(
